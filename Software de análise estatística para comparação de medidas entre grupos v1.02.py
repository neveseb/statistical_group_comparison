...
class StatisticalApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Software de análise estatística para comparação de medidas entre grupos v.1.02 - By Eduardo Borba Neves")
        ...
        try:
                ...
                # ── A2: TRÊS OU MAIS MOMENTOS/CONDIÇÕES ──────────────────────────
                    ...
                    if is_normal:
                        ...
                        try:
                            ...
                            # normaliza nomes de colunas: substitui hífen por underline para busca robusta
                            rm_cols_norm = {c.replace('-', '_'): c for c in rm.columns}
                            
                            def _get(col):
                                col_norm = col.replace('-', '_')
                                if col_norm in rm_cols_norm:
                                    real_col = rm_cols_norm[col_norm]
                                else:
                                    fallbacks = {
                                        'p_unc': ['p_GG_corr', 'p_hf_corr', 'p_corr'],
                                        'eps':   ['eps_GG',    'eps_hf']
                                    }
                                    real_col = None
                                    for alt in fallbacks.get(col_norm, []):
                                        if alt in rm_cols_norm:
                                            real_col = rm_cols_norm[alt]
                                            break
                                    if real_col is None:
                                        return np.nan
                                v = rm.loc[rm['Source'] == repeated_col, real_col].values
                                return v[0] if len(v) > 0 else np.nan
                            
                            p_rm   = _get('p-unc')   # usará p_GG_corr se p_unc ausente
                            f_rm   = _get('F')
                            ng2_rm = _get('ng2')
                            eps_rm = _get('eps')
                            ...
                            # normaliza nomes de colunas do post-hoc (hífen → underline)
                            ph_cols_norm = {c.replace('-', '_'): c for c in ph_rm.columns}
                            
                            def _ph_col(*candidates):
                                for cand in candidates:
                                    key = cand.replace('-', '_')
                                    if key in ph_cols_norm:
                                        return ph_cols_norm[key]
                                return None
                            ...
                            col_T      = _ph_col('T')
                            col_dof    = _ph_col('dof')
                            col_punc   = _ph_col('p-unc')
                            col_pcorr  = _ph_col('p-corr', 'p-adjust')
                            col_cohen  = _ph_col('cohen-d')
                            col_bf     = _ph_col('BF10')
                            
                            cols_ph = [c for c in [col_A, col_B, col_T, col_dof,
                                                    col_punc, col_pcorr, col_cohen, col_bf]
                                       if c is not None]
                            
                            output += "▸ Comparações pareadas – Bonferroni:\n"
                            output += ph_rm[cols_ph].to_string(index=False) + "\n\n"
                            
                            # interpretação par a par
                            output += "  ► Interpretação par a par:\n"
                            for _, row in ph_rm.iterrows():
                                grp_a   = row[col_A] if col_A else '?'
                                grp_b   = row[col_B] if col_B else '?'
                                t_val   = f"{row[col_T]:.3f}"       if col_T    else 'n/a'
                                p_corr  = row[col_pcorr]            if col_pcorr else np.nan
                                p_unc   = row[col_punc]             if col_punc  else np.nan
                                d_val   = row[col_cohen]            if col_cohen else np.nan
                            
                                p_show  = p_corr if not np.isnan(p_corr) else p_unc
                                sig     = "✔ Significativo" if (not np.isnan(p_show) and p_show < 0.05) else "✘ Não significativo"
                            
                                if not np.isnan(d_val):
                                    d_abs  = abs(d_val)
                                    d_interp = "grande" if d_abs >= 0.8 else "médio" if d_abs >= 0.5 else "pequeno"
                                    d_str  = f"d de Cohen = {d_val:.3f} (efeito {d_interp})"
                                else:
                                    d_str  = "d de Cohen = n/d"
                            
                                p_str   = f"{p_show:.4f}" if not np.isnan(p_show) else "n/d"
                                output += (
                                    f"    • '{grp_a}' vs '{grp_b}': t = {t_val}, "
                                    f"p_ajust = {p_str} → {sig}; {d_str}\n"
                                )
                            output += "\n"
            ...
            # ════════════════════════════════════════════════════════════════════
                ...
                if is_normal:
                    ...
                    output += (
                        f"  Justificativa: "
                        f"{'Covariáveis (' + ', '.join(controls) + ') presentes — ANCOVA remove a variância explicada por elas antes de testar o efeito principal, tornando a comparação entre grupos mais precisa e com menor erro residual.' if controls else 'Dois fatores categóricos (' + group + ' e ' + moment_col + ') com distribuição normal confirmada — ANOVA fatorial avalia os efeitos principais e a interação entre ambos os fatores simultaneamente.'}\n\n"
                    )
                    ...
                    for factor in table.index:
                        ...
                        if group in str(factor) and ':' not in str(factor):
                            p_group_b1 = p_val
                        if moment_col and moment_col in str(factor) and ':' not in str(factor):
                            p_moment_b1 = p_val
                        if ':' in str(factor):
                            p_inter_b1 = p_val

                    output += f"\n  ► Interpretação dos Resultados ({model_name}):\n"

                    # — Interação primeiro —
                    if moment_col and not np.isnan(p_inter_b1):
                        if p_inter_b1 < 0.05:
                            output += (
                                f"  [1] INTERAÇÃO '{group} × {moment_col}': SIGNIFICATIVA (p={p_inter_b1:.4f}). "
                                f"O efeito de '{group}' sobre '{target}' NÃO é uniforme — ele varia conforme o nível de '{moment_col}'. "
                                f"Isso significa que os grupos se comportam de forma distinta em diferentes momentos/condições, "
                                f"e os efeitos principais abaixo devem ser interpretados com cautela, pois são modulados por essa interação. "
                                f"Recomenda-se examinar os efeitos simples (simple effects) em cada nível de '{moment_col}'.\n"
                            )
                        else:
                            output += (
                                f"  [1] INTERAÇÃO '{group} × {moment_col}': NÃO SIGNIFICATIVA (p={p_inter_b1:.4f}). "
                                f"O padrão de diferença entre grupos em '{target}' é consistente em todos os níveis de '{moment_col}'. "
                                f"Os efeitos principais abaixo podem ser interpretados de forma direta e independente.\n"
                            )

                    # — Efeito principal do grupo —
                    sig_b1_group = (
                        "SIGNIFICATIVO (p < 0,05)"
                        if (not np.isnan(p_group_b1) and p_group_b1 < 0.05)
                        else "NÃO SIGNIFICATIVO (p ≥ 0,05)"
                    )
                    prefix_group = "[2]" if moment_col else "[1]"
                    output += (
                        f"  {prefix_group} EFEITO PRINCIPAL de '{group}': {sig_b1_group} (p={p_group_b1:.4f}). "
                    )
                    if not np.isnan(p_group_b1) and p_group_b1 < 0.05:
                        output += (
                            f"Há evidência de que pelo menos um grupo difere significativamente dos demais em '{target}', "
                            f"independentemente dos demais fatores no modelo. "
                        )
                        if controls:
                            output += (
                                f"Este efeito é avaliado após remoção da variância atribuída às covariáveis ({', '.join(controls)}), "
                                f"representando o efeito 'limpo' de '{group}' sobre '{target}'. "
                            )
                    else:
                        output += (
                            f"Não há diferença detectável entre os níveis de '{group}' em '{target}' "
                            f"após contabilizar os demais termos do modelo. "
                        )
                    output += "\n"

                    # — Efeito principal do momento —
                    if moment_col and not np.isnan(p_moment_b1):
                        sig_b1_moment = (
                            "SIGNIFICATIVO (p < 0,05)"
                            if p_moment_b1 < 0.05
                            else "NÃO SIGNIFICATIVO (p ≥ 0,05)"
                        ...
                        output += (
                            f"  [3] EFEITO PRINCIPAL de '{moment_col}': {sig_b1_moment} (p={p_moment_b1:.4f}). "
                        )
                        if p_moment_b1 < 0.05:
                            output += (
                                f"Os níveis de '{moment_col}' diferem significativamente em '{target}', "
                                f"independentemente de '{group}'. Isso indica que o momento/condição por si só "
                                f"exerce influência relevante sobre a variável resposta. "
                            )
                        else:
                            output += (
                                f"Os níveis de '{moment_col}' não diferem significativamente em '{target}', "
                                f"indicando que este fator não contribui de forma independente para a variação observada. "
                            )
                        output += "\n"
                    ...
                    if not np.isnan(p_group_b1) and p_group_b1 < 0.05 and n_groups > 2:
                        ...
                        output += (
                            f"\n  ► O Tukey HSD controla o erro do Tipo I para todas as comparações simultâneas. "
                            f"Pares com p ajustado < 0,05 diferem significativamente em '{target}'. "
                            f"O d de Cohen quantifica a magnitude prática da diferença: "
                            f"pequeno (d < 0,5), médio (0,5 ≤ d < 0,8) e grande (d ≥ 0,8).\n\n"
                        )
                    ...
                    if controls:
                        ...
                        output += (
                            f"  Justificativa: Dados não-normais com covariáveis presentes. "
                            f"A ANCOVA baseada em postos (Conover & Iman, 1982) converte os valores de '{target}' "
                            f"e das covariáveis em ranks antes de aplicar a ANCOVA paramétrica. "
                            f"Esta abordagem é robusta a desvios severos da normalidade e preserva a capacidade "
                            f"de controlar o efeito de variáveis confundidoras, sem as restrições do modelo paramétrico.\n\n"
                        )
                        ...
                        for factor in table_rank.index:
                            ...
                            if group in str(factor) and ':' not in str(factor):
                                p_group_b2a = p_val_r
                            if moment_col and moment_col in str(factor) and ':' not in str(factor):
                                p_moment_b2a = p_val_r
                            if ':' in str(factor):
                                p_inter_b2a = p_val_r

                        output += f"\n  ► Interpretação dos Resultados (ANCOVA sobre Postos):\n"

                        # — Interação primeiro —
                        if moment_col and not np.isnan(p_inter_b2a):
                            if p_inter_b2a < 0.05:
                                output += (
                                    f"  [1] INTERAÇÃO '{group} × {moment_col}': SIGNIFICATIVA (p={p_inter_b2a:.4f}). "
                                    f"O efeito de '{group}' sobre os postos de '{target}' varia em função de '{moment_col}'. "
                                    f"A relação entre grupos não é homogênea entre os momentos/condições avaliados, "
                                    f"e os efeitos principais devem ser interpretados com cautela. "
                                    f"Recomenda-se análise de efeitos simples estratificados por '{moment_col}'.\n"
                                )
                            else:
                                output += (
                                    f"  [1] INTERAÇÃO '{group} × {moment_col}': NÃO SIGNIFICATIVA (p={p_inter_b2a:.4f}). "
                                    f"O padrão de diferença entre grupos nos postos de '{target}' é consistente "
                                    f"em todos os níveis de '{moment_col}'. Os efeitos principais podem ser "
                                    f"interpretados de forma independente.\n"
                                )

                        # — Efeito principal do grupo —
                        sig_b2a_group = (
                            "SIGNIFICATIVO (p < 0,05)"
                            if (not np.isnan(p_group_b2a) and p_group_b2a < 0.05)
                            else "NÃO SIGNIFICATIVO (p ≥ 0,05)"
                        )
                        prefix_b2a_group = "[2]" if moment_col else "[1]"
                        output += (
                            f"  {prefix_b2a_group} EFEITO PRINCIPAL de '{group}': {sig_b2a_group} (p={p_group_b2a:.4f}). "
                        )
                        if not np.isnan(p_group_b2a) and p_group_b2a < 0.05:
                            output += (
                                f"Há evidência de diferença na distribuição de postos de '{target}' entre os níveis de '{group}', "
                                f"após controle das covariáveis rank-transformadas ({', '.join(controls)}). "
                                f"O efeito observado reflete diferenças reais na ordenação dos indivíduos e não "
                                f"pode ser atribuído às variáveis controladas. "
                            )
                        else:
                            output += (
                                f"Não há diferença detectável nos postos de '{target}' entre os níveis de '{group}' "
                                f"após controle das covariáveis ({', '.join(controls)}). "
                            )
                        output += "\n"

                        # — Efeito principal do momento —
                        if moment_col and not np.isnan(p_moment_b2a):
                            sig_b2a_moment = (
                                "SIGNIFICATIVO (p < 0,05)"
                                if p_moment_b2a < 0.05
                                else "NÃO SIGNIFICATIVO (p ≥ 0,05)"
                            )
                            output += (
                                f"  [3] EFEITO PRINCIPAL de '{moment_col}': {sig_b2a_moment} (p={p_moment_b2a:.4f}). "
                            )
                            if p_moment_b2a < 0.05:
                                output += (
                                    f"Os níveis de '{moment_col}' diferem significativamente nos postos de '{target}', "
                                    f"independentemente de '{group}'. O momento/condição exerce influência própria "
                                    f"e independente sobre a variável resposta. "
                                )
                            else:
                                output += (
                                    f"Os níveis de '{moment_col}' não diferem significativamente nos postos de '{target}', "
                                    f"indicando que este fator não contribui de forma independente para a variação observada. "
                                )
                            output += "\n"
                        ...
                        if not np.isnan(p_group_b2a) and p_group_b2a < 0.05 and n_groups > 2:
                            ...
                            output += (
                                f"  ► O Tukey HSD aplicado sobre os postos de '{target}' controla o erro Tipo I "
                                f"para todas as comparações simultâneas entre níveis de '{group}'. "
                                f"Pares com p ajustado < 0,05 diferem significativamente na distribuição de postos de '{target}', "
                                f"indicando diferença real na ordenação dos valores entre esses grupos após controle das covariáveis.\n\n"
                            )
                        ...
                        output += (
                            ...
                            f"('{group}' e '{moment_col}'). O teste de Scheirer-Ray-Hare (1976) é a extensão "
                            f"não-paramétrica da ANOVA fatorial de dois fatores: converte os dados em ranks globais "
                            f"e decompõe a variância dos postos entre os fatores e sua interação, sem assumir "
                            f"normalidade ou homogeneidade de variâncias.\n\n"
                        )
                        ...
                        for fac, res_srh in srh_results.items():
                            ...
                            if moment_col and fac == moment_col:
                                srh_moment_p      = res_srh['p']
                                srh_moment_H      = res_srh['H']
                                srh_moment_df     = res_srh['df']
                                srh_moment_eta    = eta2H
                                srh_moment_interp = interp_srh

                        output += f"\n  ► Interpretação dos Resultados (Scheirer-Ray-Hare):\n"

                        # — Interação primeiro —
                        if moment_col and not np.isnan(srh_inter_p):
                            if srh_inter_p < 0.05:
                                output += (
                                    f"  [1] INTERAÇÃO '{group} × {moment_col}': SIGNIFICATIVA "
                                    f"[H({int(srh_inter_df)})={srh_inter_H:.3f}, p={srh_inter_p:.4f}]. "
                                    f"O efeito de '{group}' sobre os postos de '{target}' NÃO é uniforme — "
                                    f"ele se modifica em função dos níveis de '{moment_col}'. "
                                    f"A presença de interação significa que cada combinação de grupo × momento "
                                    f"produz um padrão único de resposta, e os efeitos principais isolados "
                                    f"são insuficientes para descrever o fenômeno. "
                                    f"Recomenda-se análise de efeitos simples (comparações dentro de cada nível de '{moment_col}').\n"
                                )
                            else:
                                output += (
                                    f"  [1] INTERAÇÃO '{group} × {moment_col}': NÃO SIGNIFICATIVA "
                                    f"[H({int(srh_inter_df)})={srh_inter_H:.3f}, p={srh_inter_p:.4f}]. "
                                    f"Os fatores atuam de forma aditiva e independente — o padrão de diferença "
                                    f"entre grupos em '{target}' é consistente em todos os níveis de '{moment_col}'. "
                                    f"Os efeitos principais abaixo podem ser interpretados sem restrições.\n"
                                )
                        ...
                        prefix_srh_group = "[2]" if moment_col else "[1]"
                        output += (
                            f"  {prefix_srh_group} EFEITO PRINCIPAL de '{group}': {sig_srh_group} "
                            f"[H({int(srh_group_df)})={srh_group_H:.3f}, p={srh_group_p:.4f}, "
                            f"η²H={srh_group_eta:.3f} — efeito {srh_group_interp.lower()}]. "
                        )
                        if not np.isnan(srh_group_p) and srh_group_p < 0.05:
                            output += (
                                f"Há evidência de diferença na distribuição de postos de '{target}' entre os níveis de '{group}', "
                                f"independentemente de '{moment_col}'. "
                                f"O η²H de {srh_group_eta:.3f} indica que {srh_group_eta*100:.1f}% da variância dos postos "
                                f"é atribuível a este fator — magnitude {srh_group_interp.lower()} segundo os critérios de Cohen. "
                            )
                        else:
                            output += (
                                f"Não há diferença detectável nos postos de '{target}' entre os níveis de '{group}', "
                                f"independentemente dos níveis de '{moment_col}'. "
                            )
                        output += "\n"

                        # — Efeito principal do momento —
                        if moment_col and not np.isnan(srh_moment_p):
                            sig_srh_moment = (
                                "SIGNIFICATIVO (p < 0,05)"
                                if srh_moment_p < 0.05
                                else "NÃO SIGNIFICATIVO (p ≥ 0,05)"
                            )
                            output += (
                                f"  [3] EFEITO PRINCIPAL de '{moment_col}': {sig_srh_moment} "
                                f"[H({int(srh_moment_df)})={srh_moment_H:.3f}, p={srh_moment_p:.4f}, "
                                f"η²H={srh_moment_eta:.3f} — efeito {srh_moment_interp.lower()}]. "
                            )
                            if srh_moment_p < 0.05:
                                output += (
                                    f"Os níveis de '{moment_col}' diferem significativamente nos postos de '{target}', "
                                    f"independentemente de '{group}'. O fator tempo/condição exerce influência própria "
                                    f"e relevante sobre a variável resposta, explicando {srh_moment_eta*100:.1f}% "
                                    f"da variância dos postos (efeito {srh_moment_interp.lower()}). "
                                )
                            else:
                                output += (
                                    f"Os níveis de '{moment_col}' não diferem significativamente nos postos de '{target}', "
                                    f"indicando que o fator tempo/condição não contribui de forma independente "
                                    f"para a variação observada nesta amostra. "
                                )
                            output += "\n"
                        ...
                        if not np.isnan(srh_group_p) and srh_group_p < 0.05 and n_groups > 2:
                            ...
                            try:
                                ...
                                output += (
                                    f"  ► O teste de Dunn com correção de Bonferroni realiza comparações múltiplas "
                                    f"baseadas nos postos globais, controlando o erro Tipo I de forma conservadora. "
                                    f"Valores p < 0,05 indicam diferença significativa na distribuição de postos de "
                                    f"'{target}' entre o par avaliado. Para cada par significativo, recomenda-se "
                                    f"calcular o r de rank-biserial como medida de tamanho de efeito.\n\n"
                                )
                            except ImportError:
                                ...
                                for i_g, g1 in enumerate(groups_list):
                                    for g2 in groups_list[i_g+1:]:
                                        ...
                                        output += (
                                            f"  {sig_ph} '{g1}' vs '{g2}': "
                                            f"U={u_ph:.1f}, p_ajust={p_adj_ph:.4f} (Bonferroni, {n_comp_srh} comparações), "
                                            f"r={r_rb_ph:.3f} ({interp_r_ph}). "
                                            f"{'Diferença significativa detectada.' if p_adj_ph < 0.05 else 'Sem diferença significativa após correção.'}\n"
                                        )
                                output += "\n"
            # ════════════════════════════════════════════════════════════════════
            ...
            output += "━"*75 + "\n\n"
            output += (
                "CITAÇÃO DO SOFTWARE: NEVES, Eduardo Borba. Software de análise estatística para comparação de medidas entre grupos. Versão 1.01. [S.l.]: Zenodo, 2026. Software. DOI:10.5281/zenodo.19713014\n\n\n"
